...
## בונוס: בנק מוצרים משותף בין מכשירים
שלב 1 (יצירת KV) **גם מפעיל אוטומטית** בנק מוצרים/ספקים משותף בין כל המכשירים
(דרך `/api/data`) — בלי שום הגדרה נוספת מעבר ל-KV. ברגע שיש KV: המכשיר הראשון
"זורע" את הבנק שלו לשרת, וכל מכשיר אחר מאמץ אותו אוטומטית, וכל שינוי (מוצר/ספק)
מסתנכרן. עד שיש KV — כל מכשיר עובד מקומית כרגיל.

## שלב 1 — ליצור מאגר KV (לאחסון המנויים + הבנק המשותף)
